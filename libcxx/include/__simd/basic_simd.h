...
public:
  ...
  // TODO: mask flag constructor

  template <ranges::contiguous_range _Range, class... _Flags>
  _LIBCPP_HIDE_FROM_ABI constexpr basic_simd(_Range&& __range, simd_flags<_Flags...> = {})
    requires(ranges::size(__range) == size())
  {
    ...
    auto* __ptr =
        std::__assume_aligned<__get_align_for<value_type, _Flags...>>(std::to_address(ranges::begin(__range)));
_LIBCPP_DIAGNOSTIC_PUSH
_LIBCPP_CLANG_DIAGNOSTIC_IGNORED("-Wpsabi")
...
_LIBCPP_DIAGNOSTIC_POP
  ...
  // TODO: Range mask constructor

  // [simd.subscr]
  _LIBCPP_HIDE_FROM_ABI constexpr value_type operator[](__simd_size_type __index) const {
