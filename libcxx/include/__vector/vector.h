...
public:
  ...
  _LIBCPP_CONSTEXPR_SINCE_CXX20 _LIBCPP_HIDE_FROM_ABI iterator erase(const_iterator __first, const_iterator __last) {
    ...
    if (__first != __last) {
      ...
      //
      // We really only need is_nothrow_reocatable, but vector::erase is currently overspecified to use assignment.
      ...
      if constexpr (__is_trivially_relocatable_v<value_type> &&
        ...
        __layout_.__set_bound_using_pointer(end() - __hole_size);
